...
impl Material for ForestMaterial {
    fn fragment_shader() -> ShaderRef { "shaders/map_forest.wgsl".into() }
}
...
impl Material for RoadMaterial {
    fn fragment_shader() -> ShaderRef { "shaders/map_roads.wgsl".into() }
    fn alpha_mode(&self) -> AlphaMode { AlphaMode::Blend }
}

pub fn spawn(
    commands: &mut Commands, prepared: PreparedScenery, width: f32,
    meshes: &mut Assets<Mesh>, forest: Handle<ForestMaterial>, road: Handle<RoadMaterial>,
) {
    for (center, mesh) in prepared.forests {
        let mesh = meshes.add(mesh);
        for copy in -2..=2 {
            let offset = copy as f32*width;
            commands.spawn((Mesh3d(mesh.clone()), MeshMaterial3d(forest.clone()),
                Transform::from_xyz(offset,0.,0.), SceneryChunk(center+Vec2::X*offset),
                RenderLayers::layer(1), Visibility::Hidden));
        }
    }
    for (center, mesh) in prepared.roads {
        ...
        for copy in -2..=2 {
            let offset = copy as f32*width;
            commands.spawn((Mesh3d(mesh.clone()), MeshMaterial3d(road.clone()),
                Transform::from_xyz(offset,0.,0.), SceneryChunk(center+Vec2::X*offset),
                RenderLayers::layer(1), Visibility::Hidden));
        }
    }
...
pub fn update_visibility(
    state: Res<MapViewState>, controller: Res<MapCameraController>,
    mut chunks: Query<(&SceneryChunk, &mut Visibility)>,
) {
    if !controller.is_changed() && !state.is_changed() { return; }
    for (chunk, mut visibility) in &mut chunks {
        let visible = state.enabled && controller.distance < 280.
