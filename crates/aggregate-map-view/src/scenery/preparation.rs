use super::{PreparedScenery, geometry::{SceneryGeometry,ground}};
...
fn records(root: &Path, name: &str, magic: &[u8;4]) -> Result<Vec<[f32;4]>,String> {
    let bytes = std::fs::read(root.join("gfx/map/derived").join(name)).map_err(|error| format!("{name}: {error}"))?;
    if bytes.len() < 12 || &bytes[..4] != magic || bytes[4..8] != 1u32.to_le_bytes() {
        return Err(format!("Invalid scenery header: {name}"));
    }
    let count = u32::from_le_bytes(bytes[8..12].try_into().unwrap()) as usize;
    if bytes.len()-12 != count*16 { return Err(format!("Invalid scenery length: {name}")); }
    bytes[12..].chunks_exact(16).map(|record| {
        let values = std::array::from_fn(|i| f32::from_le_bytes(record[i*4..i*4+4].try_into().unwrap()));
        if values.iter().all(|value| value.is_finite()) { Ok(values) }
        else { Err(format!("Non-finite scenery: {name}")) }
    }).collect()
}

fn land(map: &WorldMap, position: Vec2) -> bool {
    map.provinces.index_at_uv(position/map.terrain.size)
...
fn chunk(chunks: &mut Chunks, position: Vec2) -> &mut SceneryGeometry {
    chunks.entry(((position.x/CHUNK_SIZE) as i32,(position.y/CHUNK_SIZE) as i32)).or_default()
}

fn finish(chunks: Chunks) -> Vec<(Vec2,Mesh)> {
    chunks.into_iter().map(|((x,z),geometry)| (
        Vec2::new(x as f32+0.5,z as f32+0.5)*CHUNK_SIZE,geometry.into_mesh()
    )).collect()
}
...
pub fn prepare(root: &Path, map: &WorldMap) -> Result<PreparedScenery,String> {
    ...
    for [u,v,height,kind] in records(root,"forest_instances.bin",b"AGTF")? {
        ...
        if !(0. ..1.).contains(&u) || !(0. ..1.).contains(&v) || !land(map,position) { continue; }
        let base = ground(&map.terrain,position);
        let radius = height*if kind < 0.5 { 0.38 } else { 0.65 };
        let geometry = chunk(&mut forests,position);
        let color = if kind < 0.5 { [0.12,0.22,0.085,1.] } else { [0.20,0.29,0.095,1.] };
    ...
    for [u,v,end_u,end_v] in records(root,"road_segments.bin",b"AGRD")? {
        ...
        if !(0.01..20.).contains(&length) { continue; }
        let steps = (length/0.4).ceil() as u32;
        if (0..=steps).any(|step| !land(map,start.lerp(end,step as f32/steps as f32))) { continue; }
        ...
        for step in 0..steps {
            ...
            let points = [a-offset,a+offset,b-offset,b+offset].map(|point| ground(&map.terrain,point)+Vec3::Y*0.018);
            let u0 = length*step as f32/steps as f32;
            let u1 = length*(step+1) as f32/steps as f32;
            geometry.triangle([points[0],points[1],points[2]],[Vec2::new(u0,0.),Vec2::new(u0,1.),Vec2::new(u1,0.)],[1.;4]);
            geometry.triangle([points[2],points[1],points[3]],[Vec2::new(u1,0.),Vec2::new(u0,1.),Vec2::new(u1,1.)],[1.;4]);
        }
        road_count += 1;
    }
    tracing::info!(tree_count,road_count,forest_chunks=forests.len(),road_chunks=roads.len(),elapsed_ms=started.elapsed().as_millis(),"Map scenery prepared");
    Ok(PreparedScenery { forests:finish(forests),roads:finish(roads) })
}
