...
pub fn initialize_world(
    ...
    settings: &WorldInitializationSettings,
    definitions: ContentDefinitions,
) -> Result<Scenario, String> {
    settings.validate()?;
    let regions: BTreeMap<_, _> = catalog
        .regions
        .iter()
    ...
    let countries = catalog
        ...
        .map(|country| CountryState {
            ...
            name_key: Some(format!("country-{}", country.key.to_lowercase())),
        })
        .collect();
    ...
    let definition = |key: &str| {
        definitions
            .facilities
            .iter()
            .find(|definition| definition.id.0 == key)
            .ok_or_else(|| format!("missing starting facility definition {key}"))
    };
    ...
    for province in catalog
        .provinces
        .iter()
        .filter(|province| !province.water && province.owner.is_some())
    {
        let name = province
            .region
            .as_ref()
        ...
        snapshot.provinces.push(ProvinceState {
            ...
            country: province.owner.clone().expect("owned province"),
            name: format!("{name} · {:06X}", province.raster_color),
            name_key: None,
            stockpile: BTreeMap::from([
                (
                    GoodId("grain".into()),
                    population * settings.food_reserve_days,
                ),
                (
                    GoodId("timber".into()),
                    settings.construction_goods_per_province,
                ),
                (
                    GoodId("tools".into()),
                    settings.construction_goods_per_province,
        ...
        snapshot.population_groups.push(PopulationGroupState {
            id: PopulationGroupId(uuid::Uuid::now_v7()),
        ...
        let farm_output =
            farm.workers_per_level * farm.outputs_per_worker_day[&GoodId("grain".into())];
        for (definition, level) in [
            (farm, population.div_ceil(farm_output)),
            (logging, 1),
            (tools, 1),
        ] {
            snapshot.facilities.push(FacilityState {
                id: FacilityId(uuid::Uuid::now_v7()),
                province: province.id.clone(),
                definition: definition.id.clone(),
                level,
                production_priority: if definition.id == farm.id {
                    0
                } else if definition.id == logging.id {
                    10
                } else {
                    20
                },
    ...
    let scenario = Scenario {
        ...
        id: "geographic-sandbox".into(),
        name: "Geographic sandbox with authored economic starting conditions".into(),
        rules: WorldRules {
            staple_good: GoodId("grain".into()),
            consumption_per_person_day: 1,
        },
        definitions,
