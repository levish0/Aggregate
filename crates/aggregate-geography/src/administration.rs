...
impl AdministrativeIndex {
    pub fn new(catalog: &GeographyCatalog) -> Self {
        let mut index = Self {
            ...
            province_states: vec![None; catalog.provinces.len() + 1],
        };
        ...
        for (position, province) in catalog.provinces.iter().enumerate() {
            ...
            let Some(state_id) = province
                .region
                .as_ref()
                .zip(province.owner.as_ref())
            ...
            let state = index
                .states
                .get_mut(*state_id)
                .expect("catalog state indexed");
            state.province_indices.push(position as u32 + 1);
            *state
                .terrain_counts
                .entry(province.terrain.clone())
                .or_default() += 1;
