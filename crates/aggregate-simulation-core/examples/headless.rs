...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut simulation = Simulation::from_scenario(scenario.clone())?;
    ...
    let mut restored = Simulation::from_save_json(&saved)?;
    let mut replayed = Simulation::replay(scenario, &commands, days)?;
