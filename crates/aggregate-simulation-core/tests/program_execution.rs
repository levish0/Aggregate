...
fn no_program_loadout_advances_only_time_and_rejects_economy_commands() {
    ...
    let command: SimulationCommand =
        serde_json::from_str::<Vec<aggregate_programs::RecordedCommand>>(include_str!(
            "../../../scenarios/foundation.commands.json"
        ))
        .unwrap()
        .remove(0)
        .command;
    assert!(
        simulation
            ...
            .unwrap_err()
            .to_string()
            .contains("aggregate.economy")
    );
    assert!(simulation.command_log().is_empty());
    let save = simulation.save_json().unwrap();
    let mut restored =
        Simulation::from_save_json_with_programs(&save, vec![Arc::new(EconomyProgram)]).unwrap();
    assert!(
        restored.program_states().is_empty(),
        "installing economy must not enable it in a saved world"
    );
...
impl ProgramExecution for FailedPreparation {
    ...
    fn commit(&mut self, _: &mut World) -> Option<DayReport> {
        panic!("failed day must not commit")
    }
}
...
fn later_program_failure_discards_prepared_economy_and_clock_changes() {
    let mut simulation = Simulation::from_scenario_with_programs(
        fixture(),
        vec![Arc::new(FailedPreparationProgram), Arc::new(EconomyProgram)],
    )
    .unwrap();
