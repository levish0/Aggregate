...
pub fn world(provinces: usize) -> Scenario {
    ...
    for index in 0..provinces {
        let province_id: ProvinceId = format!("10000000-0000-7000-8000-{index:012x}").parse().unwrap();
        let group_id: PopulationGroupId = format!("20000000-0000-7000-8000-{index:012x}").parse().unwrap();
        let facility_id: FacilityId = format!("30000000-0000-7000-8000-{index:012x}").parse().unwrap();
        let mut province = province.clone(); province.id = province_id.clone();
        let mut group = group.clone(); group.id = group_id; group.province = province_id.clone();
        let mut facility = facility.clone(); facility.id = facility_id; facility.province = province_id;
