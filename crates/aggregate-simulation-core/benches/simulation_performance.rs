...
fn long_run(criterion: &mut Criterion) {
    if std::env::var("AGGREGATE_BENCH_LONG_RUN").as_deref() != Ok("1") {
        return;
    }
    ...
    for age in [0, 365, 36_500] {
        ...
        let snapshot = simulation.snapshot();
        println!(
            "LONG_RUN day={age} provinces={} groups={} facilities={} commands={} save_bytes={}",
            snapshot.provinces.len(),
            snapshot.population_groups.len(),
            snapshot.facilities.len(),
            simulation.command_log().len(),
            simulation.save_json().unwrap().len()
        );
        group.bench_function(BenchmarkId::new("day", age), |bench| {
            bench.iter(|| black_box(simulation.step().unwrap()))
        });
    }
    group.finish();
