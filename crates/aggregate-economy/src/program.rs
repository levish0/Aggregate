...
impl SimulationProgram for EconomyProgram {
    fn manifest(&self) -> ProgramManifest {
        ProgramManifest { id: "aggregate.economy".into(), version: Version::new(0,1,0), api_version: PROGRAM_API_VERSION, state_schema_version: 1, dependencies: vec![], conflicts: vec![] }
    }
    fn definitions(&self) -> ContentDefinitions { crate::content::definitions() }
    ...
    fn validate_state(&self, _: &WorldSnapshot, state: &Value) -> Result<(),String> {
        if state.as_object().is_some_and(|object|object.is_empty()) { Ok(()) } else { Err("economic state is stored in typed world components".into()) }
    }
    ...
    fn install(&self, world: &mut World) -> Result<Option<Box<dyn ProgramExecution>>,String> {
        ...
        Ok(Some(Box::new(EconomyExecution { prepare: crate::schedule::create_schedule(), commit })))
    }
}

struct EconomyExecution { prepare: Schedule, commit: Schedule }
impl ProgramExecution for EconomyExecution {
    fn prepare(&mut self, world: &mut World) -> Result<(),(&'static str,String)> {
        ...
        if let Some(failure) = world.resource_mut::<crate::day_work::DayWork>().failure.take() { return Err(failure); }
        Ok(())
    }
    fn commit(&mut self, world: &mut World) -> Option<DayReport> {
        self.commit.run(world);
        world.resource_mut::<crate::day_work::DayWork>().report.take()
    }
    fn execute_command(&mut self,world: &mut World,command: &SimulationCommand,sequence: u64,scenario: &Scenario) -> Result<CommandOutcome,SimulationError> {
