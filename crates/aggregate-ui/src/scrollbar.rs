    ...
    for region in &regions {
        let track = commands
            .spawn((
                ...
                BackgroundColor(Color::WHITE.with_alpha(0.06)),
                ScrollbarTrack { region },
                ZIndex(5),
            ))
            .id();
        let thumb = commands
            .spawn((
                ...
                BackgroundColor(theme::MUTED.with_alpha(0.7)),
                ScrollbarThumb { region },
            ))
            .id();
...
pub fn update(
    ...
    for (track, mut node) in &mut tracks {
        let Ok((computed, position)) = regions.get(track.region) else {
            continue;
        };
        ...
        node.display = if content > height + 1. {
            Display::Flex
        } else {
            Display::None
        };
    ...
    for (thumb, mut node) in &mut thumbs {
        let Ok((computed, position)) = regions.get(thumb.region) else {
            continue;
        };
        let height = computed.size().y * computed.inverse_scale_factor();
        let content = computed.content_size().y * computed.inverse_scale_factor();
        let thumb_height =
            (height * height / content.max(1.)).clamp(22_f32.min(height), height.max(0.));
...
pub fn interact(
    ...
    if let Some((entity, offset)) = interaction.active {
        let Some((_, track, transform, _)) = tracks
            .iter()
            .find(|(track, _, _, _)| track.region == entity)
        else {
            interaction.active = None;
            return;
        };
        let Some((_, thumb, _)) = thumbs.iter().find(|(thumb, _, _)| thumb.region == entity) else {
            return;
        };
        let Ok((computed, mut region, mut position)) = regions.get_mut(entity) else {
            return;
        };
        let top = transform.translation.y - track.size().y / 2.;
        let fraction =
            ((cursor.y - top - offset) / (track.size().y - thumb.size().y).max(1.)).clamp(0., 1.);
