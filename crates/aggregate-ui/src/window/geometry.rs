...
impl ResizeEdges {
    pub fn at(pointer: Vec2, geometry: WindowGeometry) -> Option<Self> {
        ...
        let axis = |position: f32, extent: f32| {
            if position <= 6. { -1 } else if position >= extent - 6. { 1 } else { 0 }
        };
        let edges = IVec2::new(axis(local.x, geometry.size.x), axis(local.y, geometry.size.y));
    ...
    pub fn resize(self, mut geometry: WindowGeometry, delta: Vec2, minimum: Vec2, viewport: Vec2) -> WindowGeometry {
        for axis in 0..2 {
            ...
            match self.0[axis] {
                ...
                1 => geometry.size[axis] = (geometry.size[axis] + delta[axis]).clamp(minimum, (viewport[axis] - start).max(minimum)),
                _ => {}
            }
        }
        geometry
...
mod tests {
    ...
    fn resizing_leading_corner_keeps_opposite_corner_and_minimum_size() {
        let original = WindowGeometry { position: Vec2::new(100., 120.), size: Vec2::new(400., 300.) };
        let resized = ResizeEdges(IVec2::splat(-1)).resize(original, Vec2::splat(500.), Vec2::new(200., 150.), Vec2::new(1000., 800.));
        assert_eq!(resized.size, Vec2::new(200., 150.));
        assert_eq!(resized.position + resized.size, original.position + original.size);
    }

    #[test]
    fn resizing_right_edge_keeps_height_and_stays_inside_viewport() {
        let original = WindowGeometry { position: Vec2::new(100., 120.), size: Vec2::new(400., 300.) };
        let resized = ResizeEdges(IVec2::X).resize(original, Vec2::splat(1000.), Vec2::new(200., 150.), Vec2::new(1000., 800.));
