...
impl Default for CountryFlags {
    fn default() -> Self {
        Self(ron::from_str(include_str!("../../../assets/flags/countries.ron")).expect("bundled country flag mapping"))
    }
}
...
pub fn flag(commands: &mut Commands, parent: Entity, country: CountryId, height: f32) -> Entity {
    let entity = commands.spawn((
        CountryFlag(country),
        ImageNode::default(),
        Node { width: px(height * 1.5), height: px(height), flex_shrink: 0., ..default() },
        Pickable::IGNORE,
    )).id();
...
pub fn load_flags(
    ...
    for (flag, mut image) in &mut images {
        ...
        let code = map.as_ref()
            .and_then(|map| map.0.catalog.countries.iter().find(|country| country.id == flag.0))
