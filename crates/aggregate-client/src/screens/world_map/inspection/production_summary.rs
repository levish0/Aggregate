...
pub(super) fn summarize(
    ...
    for facility in session.snapshot.facilities.iter().filter(|item| provinces.contains(&item.province)) {
        facilities.insert(&facility.id, &facility.definition);
        if let Some(definition) = definitions.get(&facility.definition) {
            summaries.entry(facility.definition.clone()).or_default().capacity += u128::from(facility.level) * u128::from(definition.workers_per_level);
        }
    }
    if let Some(report) = &session.last_report {
        for facility in &report.facilities {
            let Some(definition) = facilities.get(&facility.facility) else { continue; };
            ...
            for (good, amount) in &facility.inputs { *summary.inputs.entry(good.clone()).or_default() += u128::from(*amount); }
            for (good, amount) in &facility.outputs { *summary.outputs.entry(good.clone()).or_default() += u128::from(*amount); }
        }
    }
    summaries
}
