use crate::{management::{ManagementAction, ManagementSession, construction::construction_site, presentation}, state::InterfaceState};
use aggregate_ui::{button::UiButton, components as ui, fonts::UiFonts, icon::Icon, theme, tooltip::TooltipContent};
...
pub(super) fn build(commands: &mut Commands, parent: Entity, fonts: &UiFonts, interface: &InterfaceState, session: &ManagementSession, provinces: &BTreeSet<ProvinceId>, has_statistics: bool, sectors_only: bool) {
    ...
    for (order, definition) in session.definitions.facilities.iter().filter(|definition| !sectors_only || definition.construction_points_per_worker_day > 0).enumerate() {
        ...
        for field in [BuildingField::Level, BuildingField::Workforce, BuildingField::Output] {
            let parent = if matches!(field, BuildingField::Workforce) {
                let row = aggregate_ui::layout::row(commands, card, 5.);
                aggregate_ui::icon::icon(commands, row, Icon::Population, 14., theme::TEXT);
                row
            } else { card };
            let label = ui::text(commands, parent, fonts, "", if matches!(field, BuildingField::Level) { 18. } else { 12. }, theme::TEXT, false);
            commands.entity(label).insert(BuildingLabel(definition.id.clone(), field));
        }
        commands.entity(card).insert((BuildingCard(definition.id.clone()), TooltipContent { title: name, body: presentation::recipe_description(session, interface, definition), hint: String::new(), locking_label: interface.text("tooltip-locking"), locked_label: interface.text("tooltip-locked"), links: vec![] }));
        if owned {
            let button = ui::button(commands, card, fonts, "+", UiButton { enabled: false, ..UiButton::secondary(400 + order as u32) });
            commands.entity(button).insert(BuildingButton(definition.id.clone()));
        }
    }
...
pub fn update(
    ...
    session: Res<ManagementSession>, interface: Res<InterfaceState>,
    ...
    *previous = Some(key);
    ...
    for facility in session.snapshot.facilities.iter().filter(|item| scope.0.contains(&item.province)) { *levels.entry(facility.definition.clone()).or_default() += u128::from(facility.level); }
    for project in session.snapshot.construction_projects.iter().filter(|item| scope.0.contains(&item.province)) { *queued.entry(project.definition.clone()).or_default() += 1; }
    for (id, pending) in &session.pending_construction {
        if scope.0.contains(&pending.province) && !session.snapshot.construction_projects.iter().any(|project| project.facility_id == *id) { *queued.entry(pending.definition.clone()).or_default() += 1; }
    }
    let definitions: BTreeMap<_, _> = session.definitions.facilities.iter().map(|item| (&item.id, item)).collect();
    let quantities = |goods: &BTreeMap<aggregate_world::GoodId, u128>| goods.iter().map(|(good, amount)| format!("{} {amount}", presentation::good_name(&session, &interface, good))).collect::<Vec<_>>().join(" · ");
    ...
    for (label, mut text) in &mut labels {
        ...
        let value = match label.1 {
            BuildingField::Level => if count > 0 { format!("{level} → {}", level + count) } else { interface.format("inspection-building-level", &[("level", level.to_string())]) },
            BuildingField::Workforce => if session.last_report.is_some() { format!("{} / {}", summary.active_workers, summary.capacity) } else { format!("— / {}", summary.capacity) },
            BuildingField::Output => if session.last_report.is_none() { String::new() } else if definitions[&label.0].construction_points_per_worker_day > 0 { interface.format("inspection-construction-service", &[("work", summary.construction_points.to_string())]) } else { quantities(&summary.outputs) },
        };
    ...
    for (card, mut tooltip) in &mut cards {
        ...
        if session.last_report.is_some() {
            body.push_str(&format!("\n\n{}\n{}", interface.format("inspection-production-input", &[("inputs", quantities(&summary.inputs))]), interface.format("inspection-production-output", &[("outputs", quantities(&summary.outputs))])));
        }
        if tooltip.body != body { tooltip.body = body; }
    }
    for (entity, button, mut style) in &mut buttons {
        let site = construction_site(&session, &scope.0, definitions[&button.0]);
        if style.enabled != site.is_some() { style.enabled = site.is_some(); }
        if let Some(province) = site { commands.entity(entity).insert(ManagementAction::StartConstructionAt { province, definition: button.0.clone() }); }
        else { commands.entity(entity).remove::<ManagementAction>(); }
    }
    for mut text in &mut feedback {
        let value = if matches!(&session.feedback, crate::management::SessionFeedback::Error(_)) { presentation::feedback(&session, &interface) } else { String::new() };
        if text.0 != value { text.0 = value; }
    }
}
