...
pub fn refresh(
    ...
    fonts: Res<UiFonts>,
    roots: Query<Entity, With<NewsFeed>>,
    old: Query<Entity, With<NewsFeedContent>>,
    mut previous: Local<Option<(Entity, uuid::Uuid, u64, aggregate_localization::Language)>>,
) {
    ...
    *previous = Some(key);
    ...
    for entry in session.news.iter().rev().take(4) {
        ui::text(
            ...
            column,
            &fonts,
            interface.format("management-day", &[("day", entry.day.to_string())]),
            11.,
            theme::MUTED,
            false,
        );
        ui::text(
            ...
            column,
            &fonts,
            presentation::event_text(&session, &interface, &entry.event),
            13.,
            theme::TEXT,
            false,
        );
