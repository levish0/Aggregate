...
pub fn build(commands: &mut Commands, root: Entity) {
    ...
    commands.entity(resize).insert((
        aggregate_ui::window::WindowResizeHandle(panel),
...
pub fn refresh(
    ...
    *previous = Some(key);
    ...
    commands
        .entity(heading)
        .insert(BackgroundColor(theme::TITLE_BAR));
    commands
        .entity(heading)
        .insert(aggregate_ui::window::WindowDragHandle(root));
    ...
    if let Some(country) = country {
        ...
        commands.entity(swatch).insert(BackgroundColor(Color::srgb(
            country.color[0],
            country.color[1],
            country.color[2],
        )));
