...
pub fn build(commands: &mut Commands, parent: Entity) {
    let root = ui::node(
        ...
        Node {
            ...
            right: px(super::layout::OUTLINER_RIGHT + super::layout::OUTLINER_WIDTH + 12.),
            bottom: px(70),
            width: px(300),
...
pub fn refresh(
    ...
    mut rendered: Local<Option<NotificationRenderKey>>,
) {
    ...
    state.advance(time.delta_secs(), |sequence| {
        window.physical_cursor_position().is_some_and(|cursor| {
            cards.iter().any(|(_, toast, node, transform)| {
                toast.0 == sequence
    ...
    for click in clicks.read() {
        if let Ok(close) = close_buttons.get(click.0) {
            state.active.retain(|toast| toast.entry.sequence != close.0);
        }
    }
    let key = (
        root,
        ...
        state
            .active
            .iter()
            .map(|toast| toast.entry.sequence)
            .collect(),
    );
    ...
    *rendered = Some(key);
    ...
    for (order, toast) in state.active.iter().enumerate() {
        ...
        commands.entity(card).insert((
            NotificationToast(toast.entry.sequence),
            UiPointerBlocker,
            aggregate_ui::motion::PanelEntrance::new(Vec2::new(16., 0.)),
            UiTransform::default(),
        ));
        ...
        let text = ui::text(
            ...
            card,
            &fonts,
            presentation::notification_text(&session, &interface, &toast.entry.event),
            13.,
            theme::TEXT,
            false,
        );
        ...
        let close = super::controls::icon_button(
            &mut commands,
            card,
            &fonts,
            &interface,
            ...
            UiButton::secondary(1500 + order as u32),
        );
        commands
            .entity(close)
            .insert(DismissNotification(toast.entry.sequence));
    }
}
