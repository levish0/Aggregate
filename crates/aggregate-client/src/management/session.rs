...
#[derive(Resource)]
pub struct ManagementViewState { pub selected_province: ProvinceId }

impl ManagementSession {
    pub fn is_busy(&self) -> bool { self.pending_jobs != 0 }

    pub fn inspect_programs(&mut self, scope: &InspectionScope) -> Result<Option<Vec<InspectionSection>>, SimulationError> {
        if let Some((cached_scope, revision, result)) = &self.inspection
            && cached_scope == scope && *revision == self.revision {
            return result.clone().map(Some).map_err(SimulationError::InvalidPrograms);
        }
        if !self.is_busy() {
            self.worker.submit(WorkRequest::Inspect(scope.clone(), self.revision))?;
            self.pending_jobs += 1;
        }
        Ok(None)
    }
    ...
    pub fn foundation() -> Self {
        let scenario = parse_scenario(include_str!("../../../../scenarios/foundation.json")).expect("validated bundled scenario");
        let country = scenario.initial_state.countries[0].id.clone();
        Self::create(scenario, country, vec![std::sync::Arc::new(aggregate_economy::EconomyProgram)], false).expect("bundled world")
    }
    ...
    fn create(scenario: aggregate_world::Scenario, player_country: CountryId, programs: Vec<std::sync::Arc<dyn aggregate_programs::SimulationProgram>>, geographic: bool) -> Result<Self, SimulationError> {
    ...
    pub fn initial_view(&self) -> ManagementViewState {
        ManagementViewState { selected_province: self.snapshot.provinces[self.index.countries[&self.player_country].province_indices[0]].id.clone() }
    }

    pub fn start_construction(&mut self, province: &ProvinceId, definition: &FacilityDefinitionId) {
        let Some(recipe) = self.definitions.facilities.iter().find(|item| item.id == *definition) else {
            self.feedback = SessionFeedback::Error(SimulationError::CommandRejected(format!("unknown facility definition {definition}")));
            self.revision += 1;
            return;
        };
        let priority = self.snapshot.facilities.iter().filter(|facility| facility.definition == *definition).map(|facility| facility.production_priority).min().unwrap_or(100);
        let command = SimulationCommand::StartConstruction {
            country: self.player_country.clone(), province: province.clone(), facility: uuid::Uuid::now_v7().into(),
            definition: definition.clone(), workers: recipe.construction.max_workers, production_priority: priority,
        };
        self.submit(WorkRequest::Construction(command, definition.clone()));
    }

    pub fn step(&mut self) {
        if !self.is_busy() { self.submit(WorkRequest::Step); }
    }

    fn submit(&mut self, request: WorkRequest) {
        let result = if self.pending_jobs >= 32 {
            Err(SimulationError::CommandRejected("simulation request queue is full".into()))
        } else { self.worker.submit(request) };
        match result {
            Ok(()) => self.pending_jobs += 1,
            Err(error) => { self.running = false; self.feedback = SessionFeedback::Error(error); self.revision += 1; }
        }
    }

    fn accept(&mut self, completed: super::worker::CompletedWork) {
        self.pending_jobs = self.pending_jobs.saturating_sub(1);
        if let Some((snapshot, index)) = completed.snapshot { self.snapshot = snapshot; self.index = index; }
        match completed.result {
            ...
            Ok(WorkOutcome::Day(report)) => {
                self.news.extend(report.events.iter().filter(|event| {
                    ...
                    self.index.provinces.get(province).is_some_and(|position| self.snapshot.provinces[*position].country == self.player_country)
                }).cloned().map(|event| NewsEntry { day: report.day, event }));
                self.last_report = Some(report);
                if matches!(self.feedback, SessionFeedback::Error(_)) { self.feedback = SessionFeedback::Ready; }
            }
            Ok(WorkOutcome::Construction(definition, outcome)) => {
                self.news.push(NewsEntry { day: self.snapshot.day, event: outcome.event });
                self.feedback = SessionFeedback::ConstructionStarted(definition);
            }
            Err(error) => { self.running = false; self.feedback = SessionFeedback::Error(error); }
        }
